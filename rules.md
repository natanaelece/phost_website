...
- A exclusão administrativa de teste grátis exige confirmação e só é permitida quando a solicitação está `recusado`. A exclusão remove os eventos vinculados por cascata e faz o usuário voltar a `nao_solicitado`; nenhum outro estado pode ser excluído.

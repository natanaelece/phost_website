...
- Frontend em HTML estático e Vanilla JavaScript. NPM é permitido somente para os builds fixados do Tailwind e dos assets administrativos com esbuild; não introduza bundler em runtime, React ou Vue. Node.js 18 também é usado nas checagens de sintaxe.
...
- HTML, fontes de CSS/JavaScript e demais arquivos mutáveis da aplicação devem manter `Cache-Control`, `CDN-Cache-Control` e `Cloudflare-CDN-Cache-Control` como `no-store`; mídia pode continuar cacheável. Somente `/admin/assets/build/admin.<hash>.min.css/js`, gerado por `npm run assets:build`, pode receber cache imutável de um ano.

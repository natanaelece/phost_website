...
- HTML, fontes editáveis de CSS/JavaScript e demais arquivos mutáveis da aplicação mantêm `Cache-Control: no-store` no navegador; mídia pode continuar cacheável. Os assets gerados `/assets/build/public.<nome>.<hash>.css/js` e `/admin/assets/build/admin.<hash>.min.css/js` recebem cache imutável de um ano. Somente `/`, `/painel` e `/privacidade` admitem microcache de 60 segundos exclusivamente na Cloudflare, com `stale-while-revalidate=30`; outros intermediários continuam em `no-store`. Não amplie essa allowlist.

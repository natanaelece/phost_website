...
- Nunca imprima ou compartilhe a saída bruta de `systemctl cat premierapi` ou `systemctl show premierapi -p Environment`, pois a configuração atual contém segredos em diretivas `Environment=` do drop-in do serviço. Diagnósticos devem limitar as propriedades consultadas e redigir valores. Segredos de serviço devem evoluir para um `EnvironmentFile` externo ao repositório, propriedade de `root` e modo `0600`; migração e rotação exigem autorização e janela operacional.

...
As variáveis sensíveis de `premierapi` estão atualmente declaradas por diretivas `Environment=` no drop-in `/etc/systemd/system/premierapi.service.d/override.conf`; não existe um `.env` privado referenciado pela unit. Por isso, nunca exponha a saída bruta de `systemctl cat premierapi` nem de `systemctl show premierapi -p Environment`: esses comandos revelam os valores. Para diagnóstico, consulte somente propriedades não sensíveis, como `FragmentPath`, `DropInPaths` e `EnvironmentFiles`, e redija qualquer diretiva antes de compartilhar a saída. A evolução recomendada é migrar os segredos, em janela autorizada, para um `EnvironmentFile` fora do repositório, pertencente a `root` e com modo `0600`; qualquer exposição exige rotação das credenciais afetadas.
